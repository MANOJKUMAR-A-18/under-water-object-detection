...
def detect_objects(image, model_choice, confidence_threshold=0.25, iou_threshold=0.45):
    ...
    return annotated_image, summary, chart, f"Detection completed in {inference_time:.2f}s"
...
def build_app():
    ...
    with gr.Blocks(
        ...
        with gr.Tabs() as main_tabs:
            ...
            with gr.Tab("🔍 Detection System"):
                ...
                with gr.Row():
                    with gr.Column():
                        gr.HTML("""
                        <div style="text-align: center; padding: 0.8rem; background: linear-gradient(135deg, rgba(30,60,114,0.1), rgba(76,161,175,0.1)); border-radius: 10px; margin-bottom: 1rem;">
                            <h4 style="color: #4ca1af; margin: 0;">⚙️ Settings</h4>
                        </div>
                        """)
                        ...
                        with gr.Row():
                            ...
                            iou_slider = gr.Slider(
                                ...
                                step=0.05,
                                label="🔗 IoU Threshold", 
                                info="Overlap threshold",
                                scale=1
                            )
                
                # Main Detection Interface - Two Equal Panels
                with gr.Row(equal_height=True):
                    # Left Panel - Upload Underwater Image
                    with gr.Column(scale=1):
                        # Header with blue gradient like in reference image
                        gr.HTML("""
                        ...
                        # Image upload area with border matching reference
                        ...
                        # Controls below image - NO EXPORT BUTTON
                        with gr.Row():
                    ...
                    with gr.Column(scale=1):
                        ...
                        # Results tabs below image
                        with gr.Tabs():
                            with gr.Tab("📊 Statistics"):
                                detection_chart = gr.Plot(label="Detection Statistics")
                            
                            with gr.Tab("📝 Summary"):
                                output_summary = gr.Markdown("Upload an image to see detection results")
                            
                            with gr.Tab("📈 History"):
                                history_btn = gr.Button("🔄 Refresh History", size="sm")
                                history_output = gr.Markdown("No detection history available")
        
        
        # Event handlers
        # Navigation functionality - prevent resizing
        def navigate_to_detect():
            return gr.Tabs(selected=1)  # Use index instead of string ID
        ...
        detect_btn.click(
            ...
            outputs=[output_image, output_summary, detection_chart, status_text]
        )
